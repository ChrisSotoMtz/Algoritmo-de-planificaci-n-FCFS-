...
namespace Actividad_6
{
    public partial class Form1 : Form
    {
        ...
        {
            int idcont = 1;
            for (int i = 0; i < System.Convert.ToInt32(cantidad_tb.Text); i++)
            {
        ...
        {
            if (cantidad_tb.Text != "Cantidad de Procesos" && cantidad_tb.Text != "0")
            {
                CrearProceso(System.Convert.ToInt32(cantidad_tb.Text));
                ListarMemoria(tablaMemoria, ProcesosMemoria);
            }
            else
                MessageBox.Show("El 0 No es un elemento valido", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        ...
        #region cantidadTextboxt
        private void cantidad_tb_Enter(object sender, EventArgs e)
        {
            if (cantidad_tb.Text == "Cantidad de Procesos")
            {
                cantidad_tb.Text = "";
                cantidad_tb.ForeColor = Color.White;

            }
        }

        private void cantidad_tb_Leave(object sender, EventArgs e)
        {
            if (cantidad_tb.Text == "")
            {
                cantidad_tb.Text = "Cantidad de Procesos";
                cantidad_tb.ForeColor = Color.Silver;
            }
        }

        private void cantidad_tb_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void Form1_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyData == Keys.C && start == true && pause == true)
            {
                if (ProcesosBloqueados.Count > 0)
                    bloqueo.Start();
                timer1.Start();
